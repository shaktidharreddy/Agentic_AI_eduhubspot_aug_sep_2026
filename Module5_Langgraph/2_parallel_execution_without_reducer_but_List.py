...
def convert_to_inr(state: PortfolioState) -> dict:
    return {"total": state["total"].append( [state["total_usd"] * 95])}

def convert_to_eur(state: PortfolioState) -> dict:
    return {"total": state["total"].append( [state["total_usd"] * 0.9])}

def convert_to_rub(state: PortfolioState) -> dict:
    return {"total": state["total"].append( [state["total_usd"] * 65])}
